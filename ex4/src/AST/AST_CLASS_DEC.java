...
public class AST_CLASS_DEC extends AST_Node {
	...
	{	
		...
		if ( id2 != null)
		{
			...
			fatherDataMembers = (TYPE_LIST) fatherClass.cFieldList;
		}
	...
	public TEMP IRme() {
		if (id2 == null) {
			...
			// fields
			AST_C_FEILD_LIST temp = cFieldList;
			for (int i = 0; i < fieldlist.size(); i++) {
				...
				{
					for (AST_C_FEILD_LIST it = temp; it != null; it = it.tail) {
						if (it.head.typeCField == "varDec" &&
								((AST_C_FIELD_VAR) it.head).varDec.id.equals(n)) {
							it.head.IRme();
							temp = cFieldList;
							break;
						}
